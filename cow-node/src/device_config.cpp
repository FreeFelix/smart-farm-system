...
void initDeviceConfig() {
    ...
    deviceConfig.deviceType = DEVICE_COW_NODE; // Change per node type
    deviceConfig.deviceID = "COW-" + String((uint32_t)ESP.getEfuseMac(), HEX);
    deviceConfig.deviceName = "Cow Collar 1";
    ...
    // Cow-specific defaults
    deviceConfig.cowGender = 1; // Female by default
    deviceConfig.milkMonitoring = true; // Auto-enabled for females
    ...
    Serial.println("Default: Female cow (milk monitoring enabled)");
}
...
void printDeviceConfig() {
    ...
    Serial.print("Milk Monitoring: "); 
    Serial.println(deviceConfig.milkMonitoring ? "ENABLED (at Milk Station)" : "DISABLED");
...
void setCowGender(uint8_t gender) {
    ...
    // Auto-enable milk monitoring for females (1), disable for males (0)
    deviceConfig.milkMonitoring = (gender == 1);
    saveDeviceConfig();
    
    if (gender == 1) {
        Serial.println("Cow gender set to FEMALE - Milk monitoring ENABLED");
    } else {
        Serial.println("Cow gender set to MALE - Milk monitoring DISABLED");
    }
}
...
void processConfigCommand(const String &command) {
    ...
    if (cmd == "HELP") {
        ...
        Serial.println("SET_GENDER <F/M>        - Set cow gender (F=Female, M=Male)");
        ...
        Serial.println("");
        Serial.println("NOTE: Setting gender to FEMALE auto-enables milk");
        Serial.println("      monitoring at Milk Station. MALE disables it.");
        Serial.println("==========================================\n");
    }
    else if (cmd == "SHOW") {
        printDeviceConfig();
    ...
    else if (cmd.startsWith("SET_GENDER ")) {
        String genderStr = command.substring(11);
        genderStr.trim();
        genderStr.toUpperCase();
        
        if (genderStr == "F" || genderStr == "FEMALE") {
            setCowGender(1); // Female
        } else if (genderStr == "M" || genderStr == "MALE") {
            setCowGender(0); // Male
        } else {
            Serial.println("Invalid gender. Use F/FEMALE or M/MALE");
        }
    }
    else {
